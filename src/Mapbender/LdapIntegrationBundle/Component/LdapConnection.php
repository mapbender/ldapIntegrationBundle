...
namespace Mapbender\LdapIntegrationBundle\Component;

use IMAG\LdapBundle\Exception\ConnectionException;
use IMAG\LdapBundle\Manager\LdapConnectionInterface;
use Monolog\Logger;
...
class LdapConnection implements LdapConnectionInterface
{
    ...
    private function connect()
    {
        ...
        if ($kernel->getContainer()->hasParameter('ldap_scope_subtree')) {
            die('Got it!');
            ldap_set_option($ress, LDAP_SCOPE_SUBTREE, $kernel->getContainer()->getParameter('ldap_scope_subtree'));
        }
